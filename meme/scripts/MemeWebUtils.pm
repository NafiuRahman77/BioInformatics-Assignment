...
use lib qw(/home/glob/meme/lib/meme-5.5.5/perl);
...
unless ($@) {
  Log::Log4perl::init('/home/glob/meme/share/meme-5.5.5/logging.conf');
  $logger = Log::Log4perl->get_logger('meme.service.utils');
}

my $service_invocation_log_dir = '/home/glob/meme/var/meme-5.5.5/LOGS';
my $tmpdir = '';
...
sub update_status {
  ...
  my $template = HTML::Template->new(filename => '/home/glob/meme/share/meme-5.5.5/job_status.tmpl');
...
sub create_tar {
  my ($report, $msg_list, $page, $program, $refresh, $file_list, $log_file, $log_date, $log_args, @tar_files) = @_;
  ...
  &report_status('Tar', $status, $time, $msg_list, $page, $program, $file_list, $log_file, $log_date, $log_args) if ($report);
  return $tarname;
}
