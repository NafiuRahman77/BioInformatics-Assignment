...
async function get_job_queue_status() {
  ...
  if ("https://meme-suite.org/meme" == "https://meme-suite.org/meme") {
    const response = await fetch("https://meme-suite.org/meme-software/job-queue.json");
    if (!response.ok) {
...
MemeMenu.update_job_queue_status = async function(e) {
  try {
    ...
    let job_queue_status;
    job_queue_status = await get_job_queue_status();
    let title= "Jobs running: " + job_queue_status.running;
    title += "<br>" ;
    title += "Jobs waiting to run: " + job_queue_status.pending;
    topic.innerHTML = title;
  }
  catch {
    // We couldn't retrieve the job queue status.
